...
impl ConjoiningClauses {
    ...
    pub fn apply_fulltext<'s>(&mut self, schema: &'s Schema, where_fn: WhereFn) -> Result<()> {
        ...
        if let VariableOrPlaceholder::Variable(ref var) = b_entity {
            ...
            match self.bound_value(var) {
                Some(TypedValue::Ref(entid)) => {
                    // Generate a constraint on the datoms table to match.
                    self.constrain_column_to_entity(datoms_table_alias.clone(), DatomsColumn::Entity, entid);
                },
                Some(_) => {
                    // constrain_var_to_type should have failed the pattern. This is a coding error.
                    panic!("Incorrect type handling.");
                },
                None => {
        ...
        if let VariableOrPlaceholder::Variable(ref var) = b_value {
            ...
            // If we already have a bound value, it must be a string.
            match self.bound_value(var) {
                Some(s @ TypedValue::String(_)) => {
                    // TODO.
                    unimplemented!();
                },
                Some(_) => {
        ...
        if let VariableOrPlaceholder::Variable(ref var) = b_tx {
            ...
            // If we already have a bound value, it must be a ref.
            match self.bound_value(var) {
                Some(TypedValue::Ref(entid)) => {
                    // Generate a constraint on the datoms table to match.
                    self.constrain_column_to_entity(datoms_table_alias.clone(), DatomsColumn::Tx, entid);
                },
                Some(_) => {
                    // constrain_var_to_type should have failed the pattern. This is a coding error.
                    panic!("Incorrect type handling.");
